...
import { fetchTasks, fetchReport, BackendTask, ReportData, ReportPeriod, ReportType } from '@/lib/api';
...
function filterTasks(tasks: BackendTask[], date: string, period: ReportPeriod, type: ReportType): BackendTask[] {
...
function buildReport(filtered: BackendTask[], date: string, period: ReportPeriod, type: ReportType, ai?: Partial<ReportData>): ReportData {
  const start = periodStart(date, period);
      ...
      <div className="max-w-2xl mx-auto flex flex-col gap-5">
        ...
        <div className="flex bg-gray-200 dark:bg-slate-700 rounded-xl p-1 gap-1">
          {(['general', 'work'] as ReportType[]).map(t => (
            <button key={t} onClick={() => { setType(t); setReport(null); }}
        ...
        <div className="flex bg-gray-200 dark:bg-slate-700 rounded-xl p-1 gap-1">
          {PERIODS.map(({ key, label }) => (
            <button key={key} onClick={() => { setPeriod(key); setReport(null); }}
              className={`flex-1 py-2 text-sm font-semibold rounded-lg transition-colors ${
                period === key
                  ? 'bg-white dark:bg-slate-800 text-gray-900 dark:text-white shadow-sm'
                  : 'text-gray-400 dark:text-slate-500'
        ...
        <div className="flex items-center justify-center gap-4">
          ...
          <div className="bg-white dark:bg-slate-800 border border-gray-200 dark:border-slate-700 rounded-xl px-6 py-2.5 shadow-sm min-w-[150px] text-center">
            <span className="text-sm font-bold text-gray-900 dark:text-white">{fmt(date)}</span>
          </div>
          <button onClick={() => changeDay(1)}
            className="p-2 rounded-lg text-indigo-600 dark:text-indigo-400 hover:bg-indigo-50 dark:hover:bg-indigo-950 transition-colors">
